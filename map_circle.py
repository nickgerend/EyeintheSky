...
def dict_start_end(df, column, factor = 1., start = 0., r=False):
...
df_group_circle = df.groupby(['Same_Owner_Contractor', 'Launch Site'], sort=False)
#print(df_group_circle.size())
df_dict_circle_1 = dict_start_end(df, 'Same_Owner_Contractor', 1., 0, True) # item1
df_dict_circle_2 = dict_start_end(df, 'Launch Site', factor, inc) # item 2
#endregion
...
m_count = 50
ix = 0
#for items, row in reversed(tuple(df_group_circle)):
for items, row in df_group_circle:
    ...
    s1 = sigmoid_xy(start1, y1, start2, y2, count, 'v', 15.)
    s2 = sigmoid_xy(end1, y1, end2, y2, count, 'v', 15.)
...
for i in range(len(list_xy)):
    ...
    yi = pinch(yi,pinch_scale)
    yi = rescale(yi, ymin, yp, ymin, ymax)
    yi1 = pinch(yi1,pinch_scale)
    yi1 = rescale(yi1, ymin, yp, ymin, ymax)
    yi2 = pinch(yi2,pinch_scale)
...
# df_out.to_csv(os.path.dirname(__file__) + '/launch_linear.csv', encoding='utf-8', index=False)
#endregion
...
min_x = -x_shift
import csv
with open(os.path.dirname(__file__) + '/launch_curve.csv', 'w',) as csvfile:
