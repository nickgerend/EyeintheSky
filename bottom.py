...
df_group_circle = df.groupby(['Orbit', 'Orbit'], sort=False)
df_dict_circle_1 = dict_start_end(df, 'Orbit', 1., 0, True) # item1
df_dict_circle_2 = dict_start_end(df, 'Orbit', factor, inc, True) # item 2
#endregion
...
for items, row in df_group_circle:
    ...
    s1 = sigmoid_xy(start1, y1, start2, y2, count, 'v', 5.6)
    s2 = sigmoid_xy(end1, y1, end2, y2, count, 'v', 5.6)
...
# df_out.to_csv(os.path.dirname(__file__) + '/bottom_linear.csv', encoding='utf-8', index=False)
#endregion
...
min_x = -x_shift
import csv
with open(os.path.dirname(__file__) + '/bottom_curve.csv', 'w',) as csvfile:
    ...
    for i in range(len(list_xy)):       
        ...
        fade = (ymax-v)/y_range
        fade = x_curve(fade,1.,3.)
        #o = offset + 4.

        #6:
        o = 0.5*offset - 8.*offset*(t-min_x)/(N)
        #e:
        #o = offset - 4.*offset*(x_shift-t)/(N)
